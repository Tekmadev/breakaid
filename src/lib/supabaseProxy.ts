 ...
 * edit it): "manager" gets the full app; "viewer" (door staff on their phones)
 * is confined to the read-only /view page. A missing role counts as manager so
 * the original pre-roles account keeps working; the migration SQL backfills it.
 */
...
export async function updateSession(request: NextRequest): Promise<NextResponse> {
  ...
  if (user) {
    const role =
      (user.app_metadata as { role?: string } | undefined)?.role === "viewer"
        ? "viewer"
        : "manager";
    ...
    // (builder, employees, users admin, admin API) bounces back there.
    if (role === "viewer" && pathname !== "/view" && !pathname.startsWith("/view/")) {
